...
pub trait Cluster {
    ...
    const BOOT_NODES_MULTIADDR: &str;
}
...
async fn main() -> Result<()> {
    ...
    // TODO: Handle is Boot Node

    let keypair_path = match app_m.get_one::<String>("KEYPAIR_PATH") {
        Some(path) => path,
        None => CanaryV0::KEYPAIR_PATH,
    };

    let node_client = Node::<CanaryV0>::try_from(keypair_path);
